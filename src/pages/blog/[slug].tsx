...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const res = await fetch(
    `http://localhost/wp-json/wp/v2/posts?slug=${slug}`
  );
  const posts = await res.json();
  ...
  const authorRes = await fetch(
    `http://localhost/wp-json/wp/v2/users/${post.author}`
  );
  ...
  const catRes = await Promise.all(
    post.categories.map((id: number) =>
      fetch(
        `http://localhost/wp-json/wp/v2/categories/${id}`
      ).then((r) => r.json())
    )
  );
  ...
  const tagRes = await Promise.all(
    post.tags.map((id: number) =>
      fetch(`http://localhost/wp-json/wp/v2/tags/${id}`).then(
        (r) => r.json()
      )
    )
  );
  ...
  if (post.featured_media) {
    const imgRes = await fetch(
      `http://localhost/wp-json/wp/v2/media/${post.featured_media}`
    );
  ...
  const allRes = await fetch(
    `http://localhost/wp-json/wp/v2/posts?categories=${mainCategory}&orderby=id&order=asc&per_page=100`
  );
